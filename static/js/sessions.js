...
let _sortMode = Storage.get('Zeus-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Zeus Chat';
...
const FOLDER_STATE_KEY = 'Zeus-folder-state';
const FOLDER_ORDER_KEY = 'Zeus-folder-order';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Zeus kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Zeus Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('Zeus-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('Zeus-session-sort', mode);
  else Storage.remove('Zeus-session-sort');
  renderSessionList();
}
